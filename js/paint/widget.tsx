...
function Component() {
  ...
  let [showBrush] = useModelState<boolean>("brush");
  let [showMarker] = useModelState<boolean>("marker");
  let [showEraser] = useModelState<boolean>("eraser");
  let [showColorPicker] = useModelState<boolean>("color_picker");
  let [color, setColor] = useModelState<string>("color");
  ...
  // Keep the selected tool valid when tools are toggled off. If the current
  // tool is hidden, fall back to the first enabled one (brush -> marker ->
  // eraser -> rainbow).
  useEffect(() => {
    ...
    const fallback = ['brush', 'marker', 'eraser', 'rainbow'].find(t => enabled[t]);
    if (fallback) setTool(fallback);
  }, [tool, showBrush, showMarker, showEraser, rainbowBrush]);
  ...
  return (
    <div className="paint-container" style={{ width: `${width}px` }}>
      <div className="paint-toolbar">
        ...
        {showColorPicker && tool !== 'eraser' && tool !== 'rainbow' && (
          <input
            type="color"
            value={color}
