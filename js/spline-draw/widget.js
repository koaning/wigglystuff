...
function render({ model, el }) {
  ...
  // Track dimensions so runtime trait updates stay consistent.
  ...
  // Error display for spline computation errors
  ...
  let svg = d3.select(svg_container)
              ...
              .style("max-width", `${currentWidth}px`);
  ...
  // Spline curve overlay group (drawn above grid, below brush indicator)
  ...
  function drawCurve() {
    ...
    curveEntries.forEach(function(entry) {
      ...
      curveGroup.append("path")
          ...
          .attr("fill", "none")
          .attr("stroke", entry.color || "var(--dd-spline-color, #2563eb)")
