...
| BezierCurve | `wigglystuff.bezier_curve.BezierCurve` | `points`, `x`, `y`, `t`, `closed`, `playing`, `loop`, `interval_ms`, `duration_ms`, `sync_throttle_ms`, `x_bounds`, `y_bounds`, `width`, `height` | Arbitrary-degree Bezier curve editor with draggable control points and playback |
| CurveEditor | `wigglystuff.curve_editor.CurveEditor` | `points`, `x`, `y`, `t`, `curve`, `closed`, `playing`, `loop`, `tension`, `alpha`, `selected_index`, `x_bounds`, `y_bounds`, `width`, `height` | Chart-space curve editor with D3 line interpolators and path progress output |
