...
| ScientificNumber | `wigglystuff.scientific_number.ScientificNumber` | `value`, `scaled_value`, `raw_value`, `label`, `unit_label`, `scale`, `scale_label`, `step`, `min`, `max`, `width`, `inline_mode` | Text-field numeric input with scientific notation; `value` applies an arbitrary `scale`; `.inline()` renders at text height |
