...
| AttractorWidget | `wigglystuff.attractor.AttractorWidget` | `x_expr`, `y_expr`, `params`, `n_points`, `iterations_per_frame`, `view`, `colormap`, `color_speed`, `color_phase`, `background`, `width`, `height` | GPU-accelerated 2D iterative-map attractor renderer (Clifford, De Jong, or custom formula) with WebGL2 + Canvas2D fallback. `colormap="phase"` uses Reusser's per-iteration-distance phase coloring; the named LUTs (`magma`, `viridis`, `inferno`, `plasma`, `grayscale`) are density-only. |
