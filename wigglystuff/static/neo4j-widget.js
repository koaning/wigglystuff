...
function render({ model, el }) {
  ...
  let selectedRelIds = /* @__PURE__ */ new Set();
  const simulation = d3.forceSimulation().force("link", d3.forceLink().id((d) => d.element_id).distance(120)).force("charge", d3.forceManyBody().strength(-300)).force("center", d3.forceCenter(width / 2, height / 2)).force("collide", d3.forceCollide().radius(35)).on("tick", ticked);
  simulation.stop();
  function ticked() {
    linkGroup.selectAll(".neo4j-link").attr("x1", (d) => d.source.x).attr("y1", (d) => d.source.y).attr("x2", (d) => d.target.x).attr("y2", (d) => d.target.y);
    linkGroup.selectAll(".neo4j-link-label").attr("x", (d) => (d.source.x + d.target.x) / 2).attr("y", (d) => (d.source.y + d.target.y) / 2);
    nodeGroup.selectAll(".neo4j-node-group").attr("transform", (d) => `translate(${d.x},${d.y})`);
  }
  function rebuildGraph() {
    ...
    simLinks = rels.filter((r) => nodeMap.has(r.start_node_element_id) && nodeMap.has(r.end_node_element_id)).map((r) => ({
      ...r,
  ...
  function updateVisuals() {
    const linkSel = linkGroup.selectAll(".neo4j-link").data(simLinks, (d) => d.element_id);
    linkSel.exit().remove();
    linkSel.enter().append("line").attr("class", "neo4j-link").attr("marker-end", "url(#neo4j-arrowhead)").on("click", handleRelClick).on("mouseenter", (event, d) => showTooltip(event, formatProps(d.type, d.properties))).on("mouseleave", hideTooltip).merge(linkSel).classed("neo4j-selected", (d) => selectedRelIds.has(d.element_id));
    const linkLabelSel = linkGroup.selectAll(".neo4j-link-label").data(simLinks, (d) => d.element_id);
    linkLabelSel.exit().remove();
    linkLabelSel.enter().append("text").attr("class", "neo4j-link-label").merge(linkLabelSel).text((d) => d.type);
    const nodeGSel = nodeGroup.selectAll(".neo4j-node-group").data(simNodes, (d) => d.element_id);
    nodeGSel.exit().remove();
    const nodeEnter = nodeGSel.enter().append("g").attr("class", "neo4j-node-group").call(d3.drag().on("start", dragStarted).on("drag", dragged).on("end", dragEnded)).on("click", handleNodeClick).on("dblclick", handleNodeDblClick).on("mouseenter", (event, d) => showTooltip(event, formatProps(d.labels.join(":"), d.properties))).on("mouseleave", hideTooltip);
    nodeEnter.append("circle").attr("class", "neo4j-node").attr("r", 14).attr("fill", (d) => colorScale(d.labels[0] || ""));
    nodeEnter.append("text").attr("class", "neo4j-node-label").attr("dy", 28).attr("text-anchor", "middle");
    const merged = nodeEnter.merge(nodeGSel);
    merged.select(".neo4j-node").attr("fill", (d) => colorScale(d.labels[0] || "")).classed("neo4j-selected", (d) => selectedNodeIds.has(d.element_id));
    merged.select(".neo4j-node-label").text((d) => d.display);
  }
