...
function render({ model, el }) {
  ...
  title.textContent = "\u{1F3AE} Mopad Widget";
  ...
  function updateDpadDisplay() {
    ...
    if (dpadRight)
      directions.push("\u2192");
    dpadDisplay.textContent = directions.length > 0 ? `D-pad: ${directions.join(" ")}` : "D-pad: \u2014";
  }
  ...
  function updateTimestampDisplay() {
    if (currentTimestamp > 0) {
      ...
      const timeDiff = previousTimestamp > 0 ? ((currentTimestamp - previousTimestamp) / 1e3).toFixed(3) : "N/A";
  ...
  function run_loop() {
    ...
    if (gamepad && gamepadConnected) {
      ...
      if (newDpadUp !== dpadUp || newDpadDown !== dpadDown || newDpadLeft !== dpadLeft || newDpadRight !== dpadRight) {
        dpadUp = newDpadUp;
