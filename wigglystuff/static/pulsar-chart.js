...
function render({ model, el }) {
  ...
  let peakScale = model.get("peak_scale") || 1;
  ...
  function createSelectedRow(values) {
    return values.map((y, i) => ({
      x: xValues[i] !== void 0 ? xValues[i] : i,
      y
    }));
  }
  ...
  const xLabelText = svg.append("text").attr("class", "pulsar-axis-label pulsar-x-label").attr("x", margin.left + innerWidth / 2).attr("y", height - 5).attr("text-anchor", "middle").text(xLabel);
  const yLabelText = svg.append("text").attr("class", "pulsar-axis-label pulsar-y-label").attr("transform", "rotate(-90)").attr("x", -(margin.top + innerHeight / 2)).attr("y", 15).attr("text-anchor", "middle").text(yLabel);
  function computeScales() {
    const numRows = data.length;
    if (numRows === 0)
      return { x: null, y: null, yBand: null };
    ...
    const yBand = d3.scaleBand().domain(rowIndices).range([innerHeight, 0]).padding(0);
    ...
    const yAmplitude = d3.scaleLinear().domain([minVal - range * 0.1, maxVal + range * 0.1]).range([amplitudeRange, -amplitudeRange]);
  ...
  function drawChart() {
    ...
    yAxisGroup.selectAll(".tick").attr("transform", (d) => {
    ...
    const rows = rowsGroup.selectAll(".pulsar-row").data(sortedData, (d) => d.index).join(
      (enter) => {
        const g2 = enter.append("g").attr("class", "pulsar-row").attr("transform", (d) => {
  ...
  function updateSelection() {
    rowsGroup.selectAll(".pulsar-row").classed("pulsar-row-selected", (d) => d.index === selectedIndex);
    yAxisGroup.selectAll(".tick text").classed("pulsar-y-label-selected", (tickData) => tickData === selectedIndex);
  }
  drawChart();
  ...
  model.on("change:peak_scale", () => {
    peakScale = model.get("peak_scale") || 1;
    drawChart();
  });
