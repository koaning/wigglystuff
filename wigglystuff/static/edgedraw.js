...
function render({ model, el }) {
  ...
  svg.append("defs").append("marker").attr("id", "arrowhead").attr("viewBox", "-0 -5 10 10").attr("refX", 13).attr("refY", 0).attr("orient", "auto").attr("markerWidth", 6).attr("markerHeight", 6).append("path").attr("d", "M0,-5L10,0L0,5").attr("class", "arrow");
  const simulation = d3.forceSimulation(nodes).force("link", d3.forceLink(links).id((d) => d.id).distance(100)).force("charge", d3.forceManyBody().strength(-50)).force("center", d3.forceCenter(width / 2, height / 2)).force("collide", d3.forceCollide().radius(50)).on("tick", ticked);
  let linkGroup = svg.append("g");
  let nodeGroup = svg.append("g");
  let node = nodeGroup.selectAll(".node").data(nodes).join("circle").attr("class", "node").attr("r", 10).attr("fill", "#69b3a2").on("click", handleNodeClick);
  let labels = nodeGroup.selectAll(".label").data(nodes).join("text").attr("class", "label").attr("dx", 15).attr("dy", 4).text((d) => d.id);
  ...
  function handleNodeClick(event, d) {
    if (!selectedNode) {
      ...
      const existingForwardLink = links.find(
        (l) => l.source === selectedNode && l.target === d || l.source.id === selectedNode.id && l.target.id === d.id
      );
  ...
  function updateNodes() {
    node = nodeGroup.selectAll(".node").data(nodes).join("circle").attr("class", "node").attr("r", 10).attr("fill", "#69b3a2").on("click", handleNodeClick);
    labels = nodeGroup.selectAll(".label").data(nodes).join("text").attr("class", "label").attr("dx", 15).attr("dy", 4).text((d) => d.id);
  ...
  const drag2 = d3.drag().on("drag", function(event, d) {
    d.x = event.x;
    d.y = event.y;
    simulation.alpha(0.1).restart();
  });
